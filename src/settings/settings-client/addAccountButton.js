    ...
    axios.post('/account/api/accounts/', formData)
      .then((resp) => {
        ...
        if (err.response) {
          ...
          this.setState({ submissionState: 'failure', submissionErrorMessage: err.message })
        } else {
          this.setState({ submissionState: 'failure', submissionErrorMessage: "Something went wrong." })
        }
      })
  }
